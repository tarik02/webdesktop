...
// RestartCapture replaces the PipeWire capture and encoder pipeline while
// preserving the encoded sample stream consumed by the transport.
func (s *Service) RestartCapture(quality Quality) error {
	if err := quality.Validate(s.cfg.Profiles); err != nil {
		return err
	}
	...
	stream, err := source.AcquireStream()
	if err != nil {
		return fmt.Errorf("acquire replacement desktop capture stream: %w", err)
	}
	replacement, err := newPersistentVideoPipeline(
		stream,
		quality,
		s.cfg.Profiles[quality.Profile],
		s.cfg.Tuning,
	...
	if err != nil {
		return fmt.Errorf("build replacement video pipeline: %w", err)
	}
	if err := replacement.Start(); err != nil {
		return fmt.Errorf("start replacement video pipeline: %w", err)
	}

	s.mu.Lock()
	if !s.running || s.pipeline != current || s.source == nil {
		s.mu.Unlock()
		return errors.Join(
			errors.New("media service changed during capture restart"),
			replacement.Close(),
		)
	}
	...
	if err := current.Close(); err != nil {
		s.logger.Warn("previous video capture pipeline cleanup failed", zap.Error(err))
	}
	s.logger.Info("desktop capture restarted",
		zap.Uint32("pipewire_node_id", stream.NodeID),
		zap.String("pipewire_target_object", stream.TargetObject),
		zap.String("profile", quality.Profile),
		zap.String("option", quality.Option),
